...
/* on win32, we suppose the libVS and includes are at the location of 'tcc.exe' */
...
LIBTCCAPI int tcc_add_library(TCCState *s, const char *libraryname)
{
    ...
    const char *libs[] = { "%s/%s.def", "%s/libVS%s.def", "%s/%s.dll", "%s/libVS%s.dll", "%s/libVS%s.a", NULL };
    const char **pp = s->static_link ? libs + 4 : libs;
#elif defined TCC_TARGET_MACHO
    const char *libs[] = { "%s/libVS%s.dylib", "%s/libVS%s.a", NULL };
    const char **pp = s->static_link ? libs + 1 : libs;
#else
    const char *libs[] = { "%s/libVS%s.so", "%s/libVS%s.a", NULL };
    const char **pp = s->static_link ? libs + 1 : libs;
#endif
    while (*pp) {
